    ...
    this.client = this.basePrisma.$extends({
      ...
      model: {
        user: {
            ...
            if (auditContext?.initiatorId) {
              await this.$parent.auditLog.create({
                data: {
                  ...
                  recordData: { action: 'User created' }
                }
              });
            }
            ...
            if (auditContext?.initiatorId) {
              await this.$parent.auditLog.create({
                data: {
                  ...
                  initiatedBy: auditContext.initiatorId,
                  initiatorUsername: auditContext.initiatorUsername,
                  recordData: { action: 'User updated' }
                }
              });
            }
            ...
            if (auditContext?.initiatorId) {
              await this.$parent.auditLog.create({
                data: {
                  ...
                  initiatedBy: auditContext.initiatorId,
                  initiatorUsername: auditContext.initiatorUsername,
                  recordData: { action: 'User deleted' }
                }
              });
            }
          ...
          // Utility methods for audit logs
          async getAuditLogs(this: any, userId: string) {
            return this.$parent.auditLog.findMany({
              where: {
                recordId: userId,
                tableName: 'users'
              },
          ...
          async getAllAuditLogs(this: any) {
            return this.$parent.auditLog.findMany({
              where: {
                tableName: 'users'
              },
